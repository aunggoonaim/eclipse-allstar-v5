...
' GDI text drawing onto buffer
Public Sub DrawMiniText(ByVal hDC As Long, ByVal X, ByVal Y, ByVal text As String, Color As Long)
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo errorhandler
    ...
    Exit Sub
errorhandler:
    HandleError "DrawMiniText", "modText", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Call DrawText(TexthDC, TextX, wrapy - 22, SubString, QBColor(White))
...
wrapy = TextY - (X * 20)
'Draw the Message
Call DrawText(TexthDC, TextX, wrapy - 22, text, QBColor(White))
...
Call DrawText(TexthDC, TextX, wrapy - 8, SubString, QBColor(White))
text = Left$(text, Len(text) - wrap)
...
wrapy = TextY - (X * 20)
'Draw the Message
Call DrawText(TexthDC, TextX, wrapy - 8, text, QBColor(White))
...
Dim TextX As Long, TextX2 As Long
Dim TextY As Long
Dim Color As Long
Dim Name As String
Dim NPCNum As Long
...
Dim TBoss As Boolean
    ...
    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo errorhandler
    If Index < 1 Or Index > MAX_MAP_NPCS Then Exit Sub
    ...
    Select Case NPC(NPCNum).Behaviour
        ...
        Case NPC_BEHAVIOUR_FRIENDLY
            If NPC(NPCNum).QuestNum < 1 Then
                Color = QBColor(Green)
                Name = Trim$(NPC(NPCNum).Name) & " [สัตว์เลี้ยง]"
            Else
                Color = QBColor(Green)
                Name = Trim$(NPC(NPCNum).Name) & " [ผู้ให้เควส]"
            End If
        Case NPC_BEHAVIOUR_BOSS
            Color = QBColor(BrightRed)
            Name = Trim$(NPC(NPCNum).Name) & " [บอส]"
        Case Else
            Color = QBColor(BrightGreen)
    ...
    TextX2 = ConvertMapX(MapNpc(Index).X * PIC_X) + MapNpc(Index).xOffset + (PIC_X \ 2) - getWidth(TexthDC, (Trim$(SAY)))
    ...
    If Len(SAY) > 0 Then
        If GetTickCount Mod 20000 < 5000 Then
            ...
            Call DrawText(TexthDC, TextX2, TextY - 15, SAY, QBColor(White))
            Call DrawText(TexthDC, TextX, TextY, Name, Color)
        Else
            SAY = vbNullString
            Call DrawText(TexthDC, TextX, TextY, Name, Color)
    ...
    'If NPC(NPCNum).Behaviour = NPC_BEHAVIOUR_FRIENDLY And NPC(NPCNum).QuestNum > 0 Then
    '    Call DrawText(TexthDC, TextX, TextY, Name, Color)
    'End If
...
Sub BltActionMsg(ByVal Index As Long)
    ...
    Select Case ActionMsg(Index).Type
        ...
        If GameFPS > 45 Then
            ' แก้ไข fps ของ action msg
            If ActionMsg(Index).Y > 0 Then
                X = ActionMsg(Index).X + Int(PIC_X \ 2) - ((Len(Trim$(ActionMsg(Index).Message)) \ 2) * 8)
                Y = ActionMsg(Index).Y - Int(PIC_Y \ 4) - 2 - (ActionMsg(Index).Scroll * 0.6) ' ปรับตัวคูณตัวหลังสุด
                ActionMsg(Index).Scroll = ActionMsg(Index).Scroll + 1
            Else
                X = ActionMsg(Index).X + Int(PIC_X \ 2) - ((Len(Trim$(ActionMsg(Index).Message)) \ 2) * 8)
                Y = ActionMsg(Index).Y - Int(PIC_Y \ 4) + 18 + (ActionMsg(Index).Scroll * 0.6) ' ปรับตัวคูณตัวหลังสุด
                ActionMsg(Index).Scroll = ActionMsg(Index).Scroll + 1
            End If
        Else
            ' แก้ไข fps ของ action msg
            If ActionMsg(Index).Y > 0 Then
                X = ActionMsg(Index).X + Int(PIC_X \ 2) - ((Len(Trim$(ActionMsg(Index).Message)) \ 2) * 8)
                Y = ActionMsg(Index).Y - Int(PIC_Y \ 4) - 2 - (ActionMsg(Index).Scroll * 1.2) ' ปรับตัวคูณตัวหลังสุด
                ActionMsg(Index).Scroll = ActionMsg(Index).Scroll + 1
            Else
                X = ActionMsg(Index).X + Int(PIC_X \ 2) - ((Len(Trim$(ActionMsg(Index).Message)) \ 2) * 8)
                Y = ActionMsg(Index).Y - Int(PIC_Y \ 4) + 18 + (ActionMsg(Index).Scroll * 1.2) ' ปรับตัวคูณตัวหลังสุด
                ActionMsg(Index).Scroll = ActionMsg(Index).Scroll + 1
            End If
        End If
