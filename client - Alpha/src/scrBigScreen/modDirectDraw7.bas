...
Dim Width As Long, Height As Long
    ...
    ' cursor ภาพเมาส์ บน npc / ผู้เล่น
       newCursor = LoadCursorFromFile("data files\graphics\cursor\normal_select.ani")
...
Dim Alphas As Byte
    ...
    Alphas = Animation(AnimInstance(Index).Animation).alpha
    ...
    If Alphas > 0 Then
        ...
        bDC = DDS_Animation(Sprite).GetDC

        FoxAlphaBlend aDC, X, Y, Width, Height, bDC, sRECT.Left, sRECT.Top, Alphas, RGB(0, 0, 0), 1

        DDS_BackBuffer.ReleaseDC aDC
        DDS_Animation(Sprite).ReleaseDC bDC
    Else
...
Dim i As Long, alpha As Long
    ...
    For i = 1 To Player_HighIndex
        If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
            If ConvertMapY(GetPlayerY(i)) < ConvertMapY(MapResource(Resource_num).Y) And ConvertMapY(GetPlayerY(i)) > ConvertMapY(MapResource(Resource_num).Y) - (DDSD_Resource(Resource_sprite).lHeight) / 32 Then
                If ConvertMapX(GetPlayerX(i)) >= ConvertMapX(MapResource(Resource_num).X) - ((DDSD_Resource(Resource_sprite).lWidth / 2) / 32) And ConvertMapX(GetPlayerX(i)) <= ConvertMapX(MapResource(Resource_num).X) + ((DDSD_Resource(Resource_sprite).lWidth / 2) / 32) Then
                    alpha = 150
                Else
                    alpha = 255
                End If
    ...
    If Not screenShot Then
        Call BltResource(Resource_sprite, X, Y, rec, alpha)
    Else
...
Private Sub BltResource(ByVal Resource As Long, ByVal dX As Long, dY As Long, rec As DxVBLib.RECT, ByVal alpha As Byte)
Dim X As Long
Dim Y As Long
Dim Width As Long
Dim Height As Long
Dim destRECT As DxVBLib.RECT
Dim aDC, bDC, i As Long
    ...
    aDC = DDS_BackBuffer.GetDC
    bDC = DDS_Resource(Resource).GetDC

    FoxAlphaBlend aDC, X, Y, Width, Height, bDC, rec.Left, rec.Top, alpha, RGB(0, 0, 0), 1

    DDS_BackBuffer.ReleaseDC aDC
    DDS_Resource(Resource).ReleaseDC bDC
...
Dim i As Long, NPCNum As Long, partyIndex As Long, n As Long
...
Dim wPower As Double
    ...
    ' วาดหลอดเลือดคนในปาร์ตี้
    If party.Leader > 0 Then
        For i = 1 To MAX_PARTY_MEMBERS
            partyIndex = party.Member(i)
            If (partyIndex > 0) And (Not partyIndex = MyIndex) And (GetPlayerMap(partyIndex) = GetPlayerMap(MyIndex)) Then ' And (Not partyIndex = MyIndex)
                ' player exists
                If GetPlayerVital(partyIndex, Vitals.HP) > 0 Then ' And GetPlayerVital(partyIndex, Vitals.HP) < GetPlayerMaxVital(partyIndex, Vitals.HP) Then
                    ' lock to Player
...
Dim wPower As Double
    ...
    If SpellBuffer > 0 Then
        If Spell(PlayerSpells(SpellBuffer)).CastTime > 0 Then
            If Not Player(MyIndex).GuildName = vbNullString Then
                ...
                tmpY = GetPlayerY(MyIndex) * PIC_Y + Player(MyIndex).yOffset - 62 - (DDSD_Character(GetPlayerSprite(MyIndex)).lHeight / 64) + sHeight + 1
            Else
                ' lock to player
                tmpX = GetPlayerX(MyIndex) * PIC_X + Player(MyIndex).xOffset + 16 - (sWidth / 2)
                tmpY = GetPlayerY(MyIndex) * PIC_Y + Player(MyIndex).yOffset - 46 - (DDSD_Character(GetPlayerSprite(MyIndex)).lHeight / 64) + sHeight + 1
            End If
    ...
    ' draw own health bar
    'If Not party.num > 0 Then
    
    If GetPlayerVital(MyIndex, Vitals.HP) > 0 Then ' And GetPlayerVital(MyIndex, Vitals.HP) < GetPlayerMaxVital(MyIndex, Vitals.HP) Then
        ' lock to Player
        tmpX = GetPlayerX(MyIndex) * PIC_X + Player(MyIndex).xOffset + 16 - (sWidth / 2)
        ...
        ' draw the bar proper
        With sRECT
            If Player(MyIndex).BuffStatus(BUFF_INVISIBLE) <> BUFF_INVISIBLE Then
                .Top = 0 ' HP bar
            Else
                .Top = sHeight * 1 ' HP bar
            End If
            .Left = 0
            .Right = .Left + barWidth
            .Bottom = .Top + sHeight
...
' DrawCooldownPic Vk'Allstar
Public Sub DrawSpellBuffer()
Dim sRECT As RECT, dRECT As RECT, i As Long
Dim sWidth As Long, sHeight As Long
Dim X As Long, Y As Long
    ...
    For i = 1 To MAX_BUFF
        ...
        If Player(MyIndex).BuffStatus(i) > 0 Then
        ...
        Case BUFF_FREEZ:
            ...
            sWidth = DDSD_SpellIcon(505).lWidth / 2
            sHeight = DDSD_SpellIcon(505).lHeight
            ...
            If DDS_SpellIcon(505) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 505, DDSD_SpellIcon(505), DDS_SpellIcon(505))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 2) * PIC_Y, DDS_SpellIcon(505), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        
        Case BUFF_NOEYE:  ' 3
        
            sWidth = DDSD_SpellIcon(408).lWidth / 2
            sHeight = DDSD_SpellIcon(408).lHeight
            ...
            If DDS_SpellIcon(408) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 408, DDSD_SpellIcon(408), DDS_SpellIcon(408))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 3) * PIC_Y, DDS_SpellIcon(408), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        
        Case BUFF_TOXIN:
        
            sWidth = DDSD_SpellIcon(545).lWidth / 2
            sHeight = DDSD_SpellIcon(545).lHeight
            ...
            If DDS_SpellIcon(545) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 545, DDSD_SpellIcon(545), DDS_SpellIcon(545))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 4) * PIC_Y, DDS_SpellIcon(545), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        
        Case BUFF_FEAR:
        
            sWidth = DDSD_SpellIcon(284).lWidth / 2
            sHeight = DDSD_SpellIcon(284).lHeight
            ...
            If DDS_SpellIcon(284) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 284, DDSD_SpellIcon(284), DDS_SpellIcon(284))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 5) * PIC_Y, DDS_SpellIcon(284), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        
        Case BUFF_NOATK:
        
            sWidth = DDSD_SpellIcon(432).lWidth / 2
            sHeight = DDSD_SpellIcon(432).lHeight
            ...
            If DDS_SpellIcon(432) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 432, DDSD_SpellIcon(432), DDS_SpellIcon(432))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 6) * PIC_Y, DDS_SpellIcon(432), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        
        Case BUFF_INVISIBLE: ' 7
        
            sWidth = DDSD_SpellIcon(516).lWidth / 2
            sHeight = DDSD_SpellIcon(516).lHeight
            ...
            If DDS_SpellIcon(516) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 516, DDSD_SpellIcon(516), DDS_SpellIcon(516))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 7) * PIC_Y, DDS_SpellIcon(516), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
            
        Case BUFF_SILENT:
        
            sWidth = DDSD_SpellIcon(229).lWidth / 2
            sHeight = DDSD_SpellIcon(229).lHeight
            ...
            If DDS_SpellIcon(229) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 299, DDSD_SpellIcon(229), DDS_SpellIcon(229))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 8) * PIC_Y, DDS_SpellIcon(229), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
    ...
    If StunDuration > 0 Then
            ...
            sWidth = DDSD_SpellIcon(411).lWidth / 2
            sHeight = DDSD_SpellIcon(411).lHeight
            ...
            If DDS_SpellIcon(411) Is Nothing Then
                Call InitDDSurf("SpellIcons\" & 411, DDSD_SpellIcon(411), DDS_SpellIcon(411))
            End If
            Call Engine_BltFast(1 * PIC_X, (13 - 1) * PIC_Y, DDS_SpellIcon(411), sRECT, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
    ...
    Exit Sub
errorhandler:
    HandleError "DrawSpellBuffer", "modDirectDraw7", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub

' Draw cooldawn mode Vk'Allstar
Public Sub DrawTextCooldown()
Dim i As Long
    ...
    For i = 1 To MAX_BUFF
        ...
        Case BUFF_STUN:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 1) * PIC_Y) + 6, "มึนงง : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_FREEZ:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 2) * PIC_Y) + 6, "แช่แข็ง : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_NOEYE:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 3) * PIC_Y) + 6, "ตาบอด : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_TOXIN:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 4) * PIC_Y) + 6, "พิษแทรก : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_FEAR:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 5) * PIC_Y) + 6, "หวาดกลัว : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_NOATK:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 6) * PIC_Y) + 6, "แขนหัก : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_INVISIBLE:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 7) * PIC_Y) + 6, "ล่องหน : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case BUFF_SILENT:
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 8) * PIC_Y) + 6, "ใบ้ : " & Player(MyIndex).BuffTime(i) + 1, QBColor(White))
        Case Else:
    ...
    If StunDuration > 0 Then
        If FormatNumber((StunDuration - (StunTime / 1000)), 1) > 1 Then
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 1) * PIC_Y) + 6, "มึนงง : " & FormatNumber((StunDuration - (StunTime / 1000)), 1), QBColor(White))
        Else
            Call DrawText(TexthDC, 2 * PIC_X, ((13 - 1) * PIC_Y) + 6, "มึนงง : 0" & FormatNumber((StunDuration - (StunTime / 1000)), 1), QBColor(White))
        End If
    End If
...
Dim AttackSpeed As Long
    ...
    If Player(Index).BuffStatus(BUFF_INVISIBLE) <> BUFF_INVISIBLE Then
        Sprite = GetPlayerSprite(Index)
    Else
        Sprite = 246 ' chage number of invisible sprite
    End If
...
Dim alpha As Byte
    ...
    alpha = NPC(MapNpc(MapNpcNum).Num).alpha

    If Sprite < 1 Or Sprite > NumCharacters Then Exit Sub
    If alpha < 0 Or alpha > 255 Then Exit Sub
    
    CharacterTimer(Sprite) = GetTickCount + SurfaceTimerMax
    If Player(MyIndex).BuffStatus(BUFF_NOEYE) = BUFF_NOEYE Then NoEye = True
    ...
    'If Alpha > 0 Then
     ...
     '   FoxAlphaBlend aDC, ConvertMapX(X), ConvertMapY(Y), (DDSD_Character(Sprite).lWidth / 4), (DDSD_Character(Sprite).lHeight / 4), bDC, rec.Left, rec.Top, Alpha, RGB(0, 0, 0), 1
...
Dim rec As RECT, rec_pos As RECT
    ...
    For i = 1 To Equipment.Equipment_Count - 1
        ...
        If itemnum > 0 And itemnum <= MAX_ITEMS Then
            ...
            If itempic > 0 And itempic <= NumItems Then ' check bug equipment pic
            ...
            If itempic > 0 And itempic <= NumItems Then
                If DDSD_Item(itempic).lWidth <= 64 Then ' more than 1 frame is handled by anim sub
                    ...
                    If GetPlayerInvItemValue(MyIndex, i) > 1 Then
                        ...
                        ElseIf Amount > 1000 And Amount < 1000000 Then
...
Dim itemnum As Long, itempic As Long
    ...
    If itemnum > 0 And itemnum <= MAX_ITEMS Then
        ...
        If itempic > 0 And itempic <= NumItems Then ' fixed bug
...
Dim NoEye As Boolean
    ...
    If Not exitGame = True Then
        If Fps_Max = True Then
            If GameFPS < 50 Then
                frmMain.lblMapLoad.Visible = True
                Exit Sub
            End If
        Else
            If GameFPS < 25 Then
        ...
        If GettingMap Then
            ...
            Exit Sub
        Else
            frmMain.lblMapLoad.Visible = False
        End If
                ...
                If IsPlaying(i) And GetPlayerMap(i) = GetPlayerMap(MyIndex) Then
                    If Y = Player(i).Y Then
                        If Not i = MyIndex Then
                            If NoEye = False Then
                                If Player(i).BuffStatus(BUFF_INVISIBLE) = 0 Then
                                    Call BltPlayer(i)
                                End If
                            End If
                        End If
    ...
    ' minimap
    'If ShowMiniMap = True Then BltMiniMap
    
    ' วาดหลอดเลือดและหลอดสกิลตัวเอง
    BltBar
    ...
    If myTarget > 0 Then
        If myTargetType = TARGET_TYPE_PLAYER Then
            If Player(myTarget).BuffStatus(BUFF_INVISIBLE) = 0 Then
    ...
    ' กด M แผนที่
    ' Call DrawText(TexthDC, Camera.Left + 12, Camera.Top + 1, Trim$("กด m เปิด/ปิด แผนที่"), QBColor(BrightGreen))

    ' draw FPS and Ping
    If BFPS Then
        If Fps_Max = True Then
            If GameFPS > 59 Then
                Call DrawText(TexthDC, Camera.Right - (Len("FPS : " & GameFPS) * 8), Camera.Top + 1, Trim$("FPS : " & GameFPS), QBColor(BrightGreen))
            Else
                Call DrawText(TexthDC, Camera.Right - (Len("FPS : " & GameFPS) * 8), Camera.Top + 1, Trim$("FPS : " & GameFPS), QBColor(BrightRed))
            End If
        Else
            If GameFPS > 29 Then
                Call DrawText(TexthDC, Camera.Right - (Len("FPS : " & GameFPS) * 8), Camera.Top + 1, Trim$("FPS : " & GameFPS), QBColor(BrightGreen))
        ...
        If Ping < 100 Then
            Call DrawText(TexthDC, Camera.Right - (Len("PING : " & Ping) * 8), Camera.Top + 16, Trim$("PING : " & Ping), QBColor(BrightGreen))
        Else
            Call DrawText(TexthDC, Camera.Right - (Len("PING : " & Ping) * 8), Camera.Top + 16, Trim$("PING : " & Ping), QBColor(BrightRed))
        End If
    End If

    Call DrawTextCooldown

    ' draw cursor, player X and Y locations
    If BLoc Then
        Call DrawText(TexthDC, Camera.Left, Camera.Top + 90, Trim$("เมาส์ x : " & CurX & " y : " & CurY), QBColor(Yellow))
        Call DrawText(TexthDC, Camera.Left, Camera.Top + 105, Trim$("พิกัดยืน x : " & GetPlayerX(MyIndex) & " y : " & GetPlayerY(MyIndex)), QBColor(Yellow))
        Call DrawText(TexthDC, Camera.Left, Camera.Top + 120, Trim$(" (แผนที่ #" & GetPlayerMap(MyIndex) & ")"), QBColor(Yellow))
    End If
   ...
   If NoEye = False Then
    ...
    ' draw npc name when targeted
    If myTarget > 0 Then
        If myTargetType = TARGET_TYPE_NPC Then
            Call DrawNpcName(myTarget)
            tg = True
        End If
    End If
    ...
    ' draw npc names
    For i = 1 To Npc_HighIndex
        If NPC(MapNpc(i).Num).Behaviour <> 0 Then
            If Not NPC(MapNpc(i).Num).Behaviour = NPC_BEHAVIOUR_FRIENDLY And Not NPC(MapNpc(i).Num).QuestNum > 0 Then
                If CurX = MapNpc(i).X And CurY = MapNpc(i).Y Then
                    'If Not tg Then
                        Call DrawNpcName(i)
                    'End If
        ...
        For i = 1 To MAX_SNOWDROPS
            With DropSnow(i)
                If .Init = True Then
                    ...
                    FoxAlphaBlend aDC, ConvertMapX(.X), ConvertMapY(.Y), 32, 32, bDC, sRECT.Left, sRECT.Top, 120, RGB(0, 0, 0), 1

                    DDS_BackBuffer.ReleaseDC aDC
                    DDS_Snow.ReleaseDC bDC
