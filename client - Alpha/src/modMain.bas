...
Sub Main()
    ...
    GameName = GetVar(App.Path & "\Data Files\UpdateConfig.ini", "OPTIONS", "GameName")
...
Dim Filename As String
    ...
    ChangeStatus "กำลังเชื่อมต่อเซิฟเวอร์อัพเดท..."
    SetProgress 0
    ...
    SetProgress 50
    ChangeStatus "กำลังเรียกข้อมูลเวอร์ชั่น."
    ...
    If CurVersion < VersionCount Then
        UpToDate = 0
        ChangeStatus "เวอร์ชั่นของคุณเป็นเวอร์ชั่นเก่า !"
        VersionsToGo = VersionCount - CurVersion
        PercentToGo = 100 / (VersionsToGo * 2)
        RunUpdates
    Else
        UpToDate = 1
        ChangeStatus "เวอร์ชั่นของคุณเป็นเวอร์ชั่นปัจจุบันแล้ว !"
    ...
    Exit Sub
ErrorHandle:
    MsgBox "ข้อผิดพลาดที่ไม่คาดคิดเกิดขึ้น : " & Err.Description & vbNewLine & vbNewLine & " มันอาจเป็นไปได้ว่าการกำหนดค่าของคุณไม่ถูกต้อง.", vbCritical
    Err.Clear
    DestroyUpdater
End Sub
...
Dim CurProgress As Long
    ...
    For i = CurVersion To VersionCount
        ...
        ChangeStatus "กำลังดาวโหลดไฟล์ > " & Str(UpdateID) & "/" & Str(VersionsToGo) & "..."
        ...
        CurProgress = CurProgress + PercentToGo
        ChangeStatus "กำลังแตกไฟล์ > " & Str(UpdateID) & "/" & Str(VersionsToGo) & "..."
        SetProgress CurProgress
