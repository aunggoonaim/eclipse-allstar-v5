...
Public Sub MakeGuild(Founder_Index As Long, Name As String)
    ...
    If Player(Founder_Index).GuildFileId > 0 Then
        PlayerMsg Founder_Index, "กรุณาออกจากกิลด์เก่าก่อนที่จะสร้างกิลด์ค่ะ !", BrightRed
        Exit Sub
    End If
    ...
    If GuildSlot = 0 Or GuildFileId = 0 Then
        PlayerMsg Founder_Index, "เสียใจด้วยค่ะ คุณมีกิลด์แล้ว ไม่สามารถสร้างกิลด์ได้ !", BrightRed
        Exit Sub
    End If
    
    If Name = "" Then
        PlayerMsg Founder_Index, "กรุณาตั้งชื่อกิลด์ !", BrightRed
        Exit Sub
    End If
    ...
    If itemamount = 0 Or itemamount < 5000 Then
        PlayerMsg Founder_Index, "ต้องการเงิน 5000 ในการสร้างกิลด์.", BrightRed
        Exit Sub
    End If
    ...
    'Inform users
    PlayerMsg Founder_Index, "การสร้างกิลด์เสร็จสิ้น !", BrightGreen
    PlayerMsg Founder_Index, "ยินดีต้อนรับสู่กิลด์ " & GuildData(GuildSlot).Guild_Name & ".", BrightGreen
...
Public Sub Request_Guild_Invite(index As Long, GuildSlot As Long, Inviter_Index As Long)
    ...
    PlayerMsg Inviter_Index, "ส่งคำขอเข้าร่วมกิลแล้ว !", Green
    PlayerMsg index, Player(Inviter_Index).Name & " ได้ชวนคุณเข้าร่วมกิล " & GuildData(GuildSlot).Guild_Name & "!", Green
    PlayerMsg index, "พิม  /guild accept   ภายใน 2 นาทีเพื่อยืนยันเข้าร่วมสมาคม.", Green
    PlayerMsg index, "พิม  /guild decline  เพื่อปฏิเสธคำขอ.", Green
End Sub
...
Dim OpenSlot As Long
    ...
    OpenSlot = FindOpenGuildMemberSlot(GuildSlot)
        ...
        If OpenSlot > 0 Then
            ...
            PlayerMsg index, "ยินดีต้อนรับเข้าสู่กิล " & GuildData(GuildSlot).Guild_Name & ".", BrightGreen
            
            PlayerMsg index, "คุณสามารถคุยในกิลได้ด้วยการพิม  ; (ข้อความ) หรือเลือกแชทเมนู", BrightGreen
        ...
        Else
            ...
            PlayerMsg index, "เสียใจด้วยค่ะ กิลนี้เต็มแล้ว !", BrightRed
        End If
...
Dim i As Integer
    ...
    If OnlineIndex = index Then
        PlayerMsg index, "คุณไม่สามารถเตะตัวเองได้!", BrightRed
        Exit Sub
    End If
    ...
    If IsOnline = True Then
        ...
        PlayerMsg OnlineIndex, "คุณถูกเตะออกจากกิล!", BrightRed
        PlayerMsg index, "ผู้เล่นถูกเตะแล้ว!", BrightRed
...
Public Sub HandleGuildMsg(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If Not Player(index).GuildFileId > 0 Then
        PlayerMsg index, "ต้องการสมาคมอยู่ เพื่อสนทนากับสมาชิค !", BrightRed
        Exit Sub
    End If
    
    Call CheckForSwears(index, Msg)
    s = "[สมาคม] [" & GuildData(TempPlayer(index).tmpGuildSlot).Guild_Name & "] " & GetPlayerName(index) & " : " & Msg
