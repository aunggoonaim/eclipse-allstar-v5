        ...
        If Quest(QuestNum).Requirement(1) <= Player(index).Level Then
            ...
            If Quest(QuestNum).Requirement(2) > 0 And Quest(QuestNum).Requirement(2) <= MAX_ITEMS Then
                If HasItem(index, Quest(QuestNum).Requirement(2)) = 0 Then
                    PlayerMsg index, "คุณจะต้องมีไอเทม " & Item(Quest(QuestNum).Requirement(2)).Name & " ในการทำเควสนี้ !", BrightRed
                    Exit Function
                End If
            End If
            ...
            If Quest(QuestNum).Requirement(3) > 0 And Quest(QuestNum).Requirement(3) <= MAX_QUESTS Then
                If Player(index).PlayerQuest(Quest(QuestNum).Requirement(3)).Status = QUEST_NOT_STARTED Or Player(index).PlayerQuest(Quest(QuestNum).Requirement(3)).Status = QUEST_STARTED Then
                    PlayerMsg index, "จำเป็นจะต้องผ่านเควส " & Trim$(Quest(Quest(QuestNum).Requirement(3)).Name) & " ก่อนที่จะรับเควสนี้ !", BrightRed
                    Exit Function
                End If
            End If
            'Go on :)
            CanStartQuest = True
        Else
            PlayerMsg index, "ต้องการเลเวลที่สูงกว่านี้ในการทำเควส!", BrightRed
        End If
    Else
        PlayerMsg index, "เควส" & Trim(Quest(QuestNum).Name) & "เป็นเควสที่ไม่สามารถทำซ้ำได้ !", BrightRed
    End If
End Function
    ...
    Select Case TaskType
        Case QUEST_TYPE_GOSLAY 'Kill X amount of X npc's.
            ...
            If TargetIndex = Quest(QuestNum).Task(ActualTask).NPC Then
                ...
                'show msg
                PlayerMsg index, "เควส : " + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " " + Trim$(NPC(TargetIndex).Name) + " ที่สังหารได้.", Yellow
        ...
        Case QUEST_TYPE_GOGATHER 'Gather X amount of X item.
            If TargetIndex = Quest(QuestNum).Task(ActualTask).Item Then
                ...
                PlayerMsg index, "เควส : " + Trim$(Quest(QuestNum).Name) + " - คุณมี " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " " + Trim$(Item(TargetIndex).Name), Yellow
        ...
        Case QUEST_TYPE_GOKILL 'Kill X amount of players.
            Player(index).PlayerQuest(QuestNum).CurrentCount = Player(index).PlayerQuest(QuestNum).CurrentCount + 1
            PlayerMsg index, "เควส : " + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " ฆ่าผู้เล่นแล้ว.", Yellow
            If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                QuestMessage index, QuestNum, "Task completed", 0
                If CanEndQuest(index, QuestNum) Then
                    EndQuest index, QuestNum
        ...
        Case QUEST_TYPE_GOTRAIN 'Hit X amount of times X resource.
            If TargetIndex = Quest(QuestNum).Task(ActualTask).Resource Then
                Player(index).PlayerQuest(QuestNum).CurrentCount = Player(index).PlayerQuest(QuestNum).CurrentCount + 1
                PlayerMsg index, "เควส : " + Trim$(Quest(QuestNum).Name) + " - " + Trim$(Player(index).PlayerQuest(QuestNum).CurrentCount) + "/" + Trim$(Quest(QuestNum).Task(ActualTask).Amount) + " ครั้ง.", Yellow
                If Player(index).PlayerQuest(QuestNum).CurrentCount >= Quest(QuestNum).Task(ActualTask).Amount Then
                    QuestMessage index, QuestNum, "Task completed", 0
                    If CanEndQuest(index, QuestNum) Then
                        EndQuest index, QuestNum
...
Public Sub EndQuest(ByVal index As Long, ByVal QuestNum As Long)
    ...
    PlayerMsg index, Trim$(Quest(QuestNum).Name) & ": [ภารกิจที่สำเร็จแล้ว]", Green
