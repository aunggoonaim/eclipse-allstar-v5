...
Sub BanIndex(ByVal BanPlayerIndex As Long, ByVal BannedByIndex As Long)
    ...
    Close #F
    Call GlobalMsg(GetPlayerName(BanPlayerIndex) & " ได้ถูกแบนจากเกม " & Options.Game_Name & " โดย [Admin] " & GetPlayerName(BannedByIndex) & "!", White)
    Call AddLog(GetPlayerName(BannedByIndex) & " ได้แบน " & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)
    Call AlertMsg(BanPlayerIndex, "คุณได้ถูกแบนโดย [Admin] " & GetPlayerName(BannedByIndex) & "!")
End Sub
...
Sub ServerBanIndex(ByVal BanPlayerIndex As Long)
    ...
    Print #F, IP & "," & "Server"
    Close #F
    Call GlobalMsg(GetPlayerName(BanPlayerIndex) & " ได้ถูกแบนจากเกม " & Options.Game_Name & " โดย " & " เซิฟเวอร์ " & "!", White)
    Call AddLog("เซิฟเวอร์ " & " ได้แบนผู้เล่น " & GetPlayerName(BanPlayerIndex) & ".", ADMIN_LOG)
    Call AlertMsg(BanPlayerIndex, "คุณถูกแบนจาก " & " เซิฟเวอร์" & "!")
End Sub
...
Sub ClassChange(ByVal index As Long, ByVal ClassNum As Long, ByVal Sex As Byte)
    ...
    Dim spritecheck As Boolean
            ...
            For n = 1 To Class(ClassNum).startItemCount
                If Class(ClassNum).StartItem(n) > 0 Then
                    ...
                    If Len(Trim$(Item(Class(ClassNum).StartItem(n)).Name)) > 0 Then
                        ...
                        Call PlayerMsg(index, "คุณได้รับไอเทม " & Trim(Item(Class(ClassNum).StartItem(n)).Name), BrightGreen)
                    End If
                End If
            Next
        End If
        ...
        ' send sound
        SendActionMsg GetPlayerMap(index), "เปลี่ยนอาชีพ !", BrightGreen, 1, (Player(index).x * 32), (Player(index).y * 32) - 16
...
Sub SaveSpells()
    Dim i As Long
    Call SetStatus("กำลังบันทึกสกิล... ")
...
Dim i As Integer
    ...
    SwearWords = "ass,fuck,shit,เหี้ย,ควย,หี,พ่อมึงตาย,แม่มึงตาย,สัส,เชี่ย,ฟัก,ฟัค,กระหรี่,กะหรี่,gm กาก,เหรี้ย,จังไร"
